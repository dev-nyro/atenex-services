    ...
    if _pool is None or _pool._closed:
        try:
            ...
            _pool = await asyncpg.create_pool(
                ...
                min_size=5,
                max_size=20,
                # Deshabilitar cache de prepared statements para funcionamiento óptimo
                statement_cache_size=0,
                # Setup para automáticamente codificar/decodificar JSONB
            ...
            log.info("PostgreSQL connection pool created successfully.")
             ...
             log.error("Network/OS error creating PostgreSQL connection pool", error=str(e), host=settings.POSTGRES_SERVER, port=settings.POSTGRES_PORT, exc_info=True)
             raise
        except Exception as e:
            log.error("Failed to create PostgreSQL connection pool", error=str(e), error_type=type(e).__name__, exc_info=True)
            raise
    return _pool
...
async def log_query_interaction(
    ...
    """
    Registra una interacción de consulta y respuesta en la tabla query_logs.
    
    Args:
        user_id: ID del usuario que hizo la consulta (opcional)
        company_id: ID de la compañía
        query: Consulta del usuario
        answer: Respuesta generada
        retrieved_documents: Lista de documentos recuperados
        metadata: Metadatos adicionales de la interacción
        chat_id: ID del chat relacionado (opcional)
    
    Returns:
        UUID del registro de consulta creado
    """
    pool = await get_db_pool()
    log_id = uuid.uuid4()
    log_entry = log.bind(log_id=str(log_id), company_id=str(company_id))
    
    query_sql = """
    INSERT INTO query_logs (
        id, user_id, company_id, query, answer, 
        retrieved_documents, metadata, chat_id, 
        created_at
    ) VALUES (
        $1, $2, $3, $4, $5, $6, $7, $8, NOW()
    ) RETURNING id;
    ...
    try:
        ...
        if not result:
            log_entry.error("Failed to create query log entry, no ID returned")
...
async def create_chat(
    ...
    """
    Crea un nuevo chat asociado a un usuario y compañía.
    
    Args:
        user_id: ID del usuario propietario del chat
        company_id: ID de la compañía
        title: Título del chat (opcional)
    
    Returns:
        UUID del chat creado
    """
    pool = await get_db_pool()
    chat_id = uuid.uuid4()
    ...
    query = """
    INSERT INTO chats (
        id, user_id, company_id, title, created_at, updated_at
    ) VALUES (
        $1, $2, $3, $4, NOW(), NOW()
    ) RETURNING id;
    """
    ...
    try:
        async with pool.acquire() as connection:
            result = await connection.fetchval(
                query, chat_id, user_id, company_id, title or "Nueva conversación"
            )
            
        if result:
            log.info("Chat created successfully", chat_id=str(chat_id), user_id=str(user_id))
            return chat_id
        else:
            log.error("Failed to create chat, no ID returned", user_id=str(user_id))
            raise RuntimeError("Failed to create chat, no ID returned")
    except Exception as e:
        log.error("Failed to create chat", error=str(e), user_id=str(user_id), exc_info=True)
        raise
...
async def get_user_chats(
    ...
    """
    Obtiene la lista de chats para un usuario y compañía.
    
    Args:
        user_id: ID del usuario
        company_id: ID de la compañía
        limit: Número máximo de chats a retornar
        offset: Número de chats a saltar para paginación
    ...
    try:
        async with pool.acquire() as connection:
            rows = await connection.fetch(query, user_id, company_id, limit, offset)
            
        chats = [dict(row) for row in rows]
        log.info(f"Retrieved {len(chats)} chats for user", user_id=str(user_id))
        return chats
    except Exception as e:
        log.error("Failed to get user chats", error=str(e), user_id=str(user_id), exc_info=True)
        raise
...
async def get_chat_messages(
    ...
    """
    Obtiene los mensajes de un chat, verificando que pertenece al usuario y compañía.
    ...
    Returns:
        Tupla con (mensajes, información del chat)
    """
    pool = await get_db_pool()
    
    # Primero verificamos que el chat exista y pertenezca al usuario y compañía
    chat_query = """
    SELECT 
        id, title, created_at, updated_at 
    FROM 
        chats 
    WHERE 
        id = $1 AND user_id = $2 AND company_id = $3;
    """
    
    messages_query = """
    SELECT 
        id, role, content, retrieved_documents, created_at
    FROM 
        messages
    WHERE 
        chat_id = $1
    ...
    try:
        async with pool.acquire() as connection:
            # Verificar chat
            chat_row = await connection.fetchrow(chat_query, chat_id, user_id, company_id)
            if not chat_row:
                return [], None
            ...
            # Obtener mensajes
            message_rows = await connection.fetch(messages_query, chat_id, limit, offset)
            messages = [dict(row) for row in message_rows]
            
        log.info(f"Retrieved {len(messages)} messages for chat", chat_id=str(chat_id))
        return messages, chat_info
    except Exception as e:
        log.error("Failed to get chat messages", error=str(e), chat_id=str(chat_id), exc_info=True)
        raise
...
async def save_message(
    ...
    Args:
        ...
        retrieved_documents: Documentos recuperados (para mensajes del asistente)
    ...
    INSERT INTO messages (
        id, chat_id, role, content, retrieved_documents, created_at
    ) VALUES (
        $1, $2, $3, $4, $5, NOW()
    ) RETURNING id;
    """
    ...
    try:
        async with pool.acquire() as connection:
            # Actualizar timestamp del chat
            await connection.execute(update_chat_query, chat_id)
        ...
        if result:
            ...
            log.error("Failed to save message, no ID returned", chat_id=str(chat_id))
            raise RuntimeError("Failed to save message, no ID returned")
    except Exception as e:
        log.error("Failed to save message", error=str(e), chat_id=str(chat_id), exc_info=True)
        raise

async def delete_chat(chat_id: uuid.UUID, user_id: uuid.UUID, company_id: uuid.UUID) -> bool:
    """
    Elimina un chat, verificando que pertenece al usuario y compañía.
    ...
    Returns:
        True si el chat fue eliminado, False si no existía o no pertenecía al usuario
    """
    pool = await get_db_pool()
    ...
    try:
        async with pool.acquire() as connection:
            result = await connection.fetchval(query, chat_id, user_id, company_id)
            
        success = result is not None
        if success:
            log.info("Chat deleted successfully", chat_id=str(chat_id), user_id=str(user_id))
        else:
            log.warning("Chat not found or doesn't belong to user", chat_id=str(chat_id), user_id=str(user_id))
            
        return success
    except Exception as e:
        log.error("Failed to delete chat", error=str(e), chat_id=str(chat_id), exc_info=True)
        raise
```
