...
def get_openai_text_embedder() -> OpenAITextEmbedder:
    ...
    if not api_key_value:
        log.warning("QUERY_OPENAI_API_KEY is missing or empty!")
...
def get_milvus_retriever(document_store: MilvusDocumentStore) -> MilvusEmbeddingRetriever:
    ...
    return MilvusEmbeddingRetriever(
        document_store=document_store, top_k=settings.RETRIEVER_TOP_K
    )
...
async def run_rag_pipeline(
    ...
    run_log = log.bind(
        query=query, company_id=company_id, user_id=user_id or "N/A", chat_id=str(chat_id) if chat_id else "N/A"
    )
    ...
    try:
        ...
        run_log.error(
            "Failed to get or build Async RAG pipeline for execution", error=str(build_err)
        )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Async RAG pipeline is not available.",
        )

    retriever_top_k = top_k if top_k is not None else settings.RETRIEVER_TOP_K
    retriever_filters = {
        "field": settings.MILVUS_COMPANY_ID_FIELD,
        "operator": "==",
        "value": company_id,
    }
    run_log.debug(
        "Pipeline execution parameters set", filters=retriever_filters, top_k=retriever_top_k
    )

    # Prepare input data including retriever parameters
    pipeline_input_data: Dict[str, Any] = {
        "text_embedder": {"text": query},
        "retriever": {"filters": [retriever_filters], "top_k": retriever_top_k},
        "prompt_builder": {"query": query},
    }
    run_log.debug(
        "Constructed AsyncPipeline run arguments", data_keys=list(pipeline_input_data.keys())
    )

    try:
        # Execute the pipeline asynchronously using positional data argument
        pipeline_result = await pipeline.run_async(pipeline_input_data)
        run_log.info(
            "Haystack AsyncPipeline (embed, retrieve, prompt) executed successfully."
        )  # LLM_COMMENT_CHANGE
        ...
        generated_prompt: Optional[str] = (
            prompt_builder_output.get("prompt")
        ...
        if not generated_prompt:
            run_log.error(
                "Failed to extract prompt from prompt_builder output",
                component_output=prompt_builder_output,
            )
            generated_prompt = (
                f"Pregunta: {query}\n\n"
        ...
        try:
            ...
            run_log.error(
                "Failed to log query interaction to database after successful generation",
                error=str(log_err),
                exc_info=True,
            )
        ...
        run_log.error(
            "Milvus error during pipeline execution",
            error_code=milvus_err.code,
            error_message=milvus_err.message,
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"Vector database error: {milvus_err.message}",
        )
    except ValueError as val_err:
        run_log.error(
            "ValueError during pipeline execution",
            error=str(val_err),
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Pipeline configuration or input error: {val_err}",
        )
    except Exception as e:
...
async def check_pipeline_dependencies() -> Dict[str, str]:
    ...
    try:
        ...
        if e.code == ErrorCode.COLLECTION_NOT_FOUND:
            ...
            log.info(
                "Milvus dependency check: Collection not found (expected if empty, will be created on write)."
            )
        elif e.code == ErrorCode.UNEXPECTED_ERROR and "connect failed" in e.message.lower():
            results["milvus_connection"] = (
                f"error: Connection Failed (code={e.code}, msg={e.message})"
            )
            log.warning(
                "Milvus dependency check failed: Connection Error",
                error_code=e.code,
            ...
            results["milvus_connection"] = (
                f"error: MilvusException (code={e.code}, msg={e.message})"
            )
            log.warning(
                "Milvus dependency check failed with Milvus error",
                error_code=e.code,
                error_message=e.message,
                exc_info=False,
            )
        ...
        log.warning(
            "Milvus dependency check failed during store initialization",
            error=str(rte),
            exc_info=False,
        )
    except Exception as e:
        ...
        log.warning(
            "Milvus dependency check failed with unexpected error",
            error=str(e),
            exc_info=True,
        )
