...
*   [x] Actualizar `pymilvus` a `==2.5.3` en `query-service/pyproject.toml`.
...
*   [x] (`query-service`) Añadir `ZILLIZ_API_KEY: SecretStr` a `app/core/config.py`.
*   [x] (`query-service`) Actualizar `MILVUS_URI` (default y validador) en `app/core/config.py` a HTTPS Zilliz.
...
*   [x] (`query-service`) Modificar `connections.connect` en `app/infrastructure/vectorstores/milvus_adapter.py` para usar `token`.
